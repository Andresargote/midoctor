...
export function ConsultsList({
	...
	const handleDeleteConsult = async (consultId: string) => {
		...
		try {
			...
			setError("Ha ocurrido un error al eliminar el consultorio");
		} finally {
			setIsLoading(false);
			router.back();
		}
	...
	return (
		<>
			...
			<Dialog.Root open={action === "delete"}>
				<Dialog.Portal>
					<Dialog.Overlay className="fixed top-0 left-0 z-50 w-full h-full bg-f-black opacity-15" />
					...
					>
						...
						<Dialog.Description className="mb-6 text-sm font-light leading-relaxed text-neutral-800">
							...
							<br />
							Quieres eliminar el consultorio "
							{searchParams.get("consult_name")}".
						</Dialog.Description>
						...
						<div className="flex items-center justify-end gap-3">
							...
							<Button
								...
								disabled={isLoading}
								onMouseDown={() =>
									handleDeleteConsult(searchParams.get("consult_id") as string)
								}
							>
								Eliminar
							</Button>
					...
					{consults?.map((consult) => (
						...
						>
							<div className="flex justify-between">
								...
								<Link
									href={`mi-consultorio/?action=delete&consult_id=${consult.id}&consult_name=${consult.name}`}
								>
									<X />
								</Link>
							</div>
