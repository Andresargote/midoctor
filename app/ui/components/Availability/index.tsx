'use client';
import type { Availability as AvailabilityType, Day, Slot } from '@/app/lib/types';
...
export function Availability({ availability }: AvailabilityProps) {
          ...
          days: prev.days.map((d) =>
            d.day === day
              ? {
                  ...
                  slots: [
                    ...d.slots,
                    { start: calcNewSlotHours.start, end: calcNewSlotHours.end },
                  ],
                }
              : d
          ),
  ...
  const handleRemoveSlot = (day: string, index: number) => {
    setValues((prev) => ({
      ...prev,
      days: prev.days.map((d) =>
        d.day === day ? { ...d, slots: d.slots.filter((_, i) => i !== index) } : d
      ),
    }));
  };
            ...
            <div key={slot.start} className='flex items-center gap-2'>
              <div className='h-12 w-36'>
                <SelectV2
                  ...
                  onChange={(value) => {
                    handleChangeSlot(availability.day, i, { ...slot, start: value });
                  }}
              ...
              <div className='h-12 w-36'>
                <SelectV2
                  ...
                  onChange={(value) => {
                    handleChangeSlot(availability.day, i, { ...slot, end: value });
                  }}
